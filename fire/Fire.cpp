...
void Fire::propogateFront(double w1, double w2, double w3) {
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= N; j++) {
            for (int k = 1; k <= N; k++) {
                double nx, ny, nz;
                // components of the gradient
                nx = (grid[(i + 1) * N * N + j * 4 * N + k] - grid[(i - 1) * N * N + j * 4 * N + k]) / (2 * h);
                ny = (grid[i * N * N + (j + 1) * 4 * N + k] - grid[i * N * N + (j - 1) * 4 * N + k]) / (2 * h);
                nz = (grid[i * N * N + j * 4 * N + k + 1] - grid[i * N * N + j * 4 * N + k - 1]) / (2 * h);
                ...
                if (w1 > 0) {
                    phix = (grid[i * N * N + j * 4 * N + k] - grid[(i - 1) * N * N + j * 4 * N + k]) / h;
                } else {
                    phix = (grid[(i + 1) * N * N + j * 4 * N + k] - grid[i * N * N + j * 4 * N + k]) / h;
                }
                if (w2 > 0) {
                    phiy = (grid[i * N * N + j * 4 * N + k] - grid[i * N * N + (j - 1) * 4 * N + k]) / h;
                } else {
                    phiy = (grid[i * N * N + (j + 1) * 4 * N + k] - grid[i * N * N + j * 4 * N + k]) / h;
                }
                if (w3 > 0) {
                    phiz = (grid[i * N * N + j * 4 * N + k] - grid[i * N * N + (j - 1) * 4 * N + k]) / h;
                } else {
                    phiz = (grid[i * N * N + (j + 1) * 4 * N + k] - grid[i * N * N + j * 4 * N + k]) / h;
                }


                // update implicit surface function
                grid[i * N * N + j * 4 * N + k] = grid[i * N * N + j * 4 * N + k]
    ...
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= N; j++) {
            for (int k = 1; k <= N; k++) {
                // compute forces
                // buoyancy
                fz = alpha * (grid[i * N * N + j * 4 * N + k] - Tair);
                // vorticity
